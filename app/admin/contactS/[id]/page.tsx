...
export default function ContactDetailPage() {
    ...
    const markAsRead = async (id: number, showAlert = true) => {
        try {
            setIsUpdating(true)
            ...
            if (error) {
                console.error('Error:', error)
                if (showAlert) alert('Error al actualizar el estado')
                return
            }

            setContact(prev => prev ? { ...prev, status: 'read' } : null)
            if (showAlert) alert('Mensaje marcado como leído')
        } catch (error) {
            console.error('Error:', error)
            if (showAlert) alert('Error al actualizar el estado')
        } finally {
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <Button onClick={handleEmailReply}>
